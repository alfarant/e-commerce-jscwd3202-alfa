...
export default function Menu(){
    ...
    // let onGetData = async() => {
    //     try {
    //         // metode pertama(1): filter client side
    ...
    // ##### On Get Data Data and Filtering on server
    let onGetData = async() => {
        try {
            let response = await axios.get(`https://my-json-server.typicode.com/alfarant/jsonserver-jcwd2302/products?category=0`)
            let responseCategory = await axios.get(`https://my-json-server.typicode.com/alfarant/jsonserver-jcwd2302/category`)
    ...
    let onFilter = async(idx) => {
        let response = await axios.get(`https://my-json-server.typicode.com/alfarant/jsonserver-jcwd2302/products?category=${idx}`)
            ...
            <div className="basis-1/6">
                ...
                </div>
                {
                    category.map((value, index) => {
                        return(
                            <div lassName='my-fs-18 mt-3 ml-2' onClick={() => onFilter(index)}>{value}</div>
                        )
                    })
                }
            </div>
